...
export default function seeMore(props: SeeMoreProps) {
    ...
    const showCustomCollapsed = !!props.customCollapsed
    const CustomCollapsed = props.customCollapsed;

    const { keyboardNavigation } = useFactsStore();
    ...
    if (showCustomCollapsed) {
        return (
            <CustomCollapsed action={props.action} toggleMore={props.toggleMore} />
        )
    }
